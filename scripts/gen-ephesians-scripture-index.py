#!/usr/bin/env python3
"""Generate the Ephesians volume's cross-book Scripture Index from the book's own text.
...
Usage:  python3 scripts/gen-ephesians-scripture-index.py --write
        python3 scripts/gen-ephesians-scripture-index.py
        python3 scripts/gen-ephesians-scripture-index.py --stats
"""
...
BOOKDIR = ROOT / "books" / "bible" / "ephisian"
...
SOURCES = [
    ...
    ("00a-ephesians-position.md",       "卷首·定位"),
    ("00b-sit-walk-stand.md",           "卷首·骨幹"),
    ("00c-revelation-order.md",         "卷首·啟示的次序"),
    ("elder-wong-systematic-study.md",  "卷首·領受總綱"),
] + [(f"{i:02d}", f"第{i}章") for i in range(1, 12)] + [
    ("99-heavenlies-and-earth.md",      "卷末"),
    ("999-afterword.md",                "跋"),
]
    ...
    for fname, label in SOURCES:
        ...
        for line in p.read_text(encoding="utf-8").splitlines():
            if line.lstrip().startswith(SKIP_PREFIX):
                continue
            # Ephesians itself is the book; drop its own explicit
            # self-references ("弗1:4", "以弗所書 2:8") before parsing
            # cross-book hits.
            cleaned = re.sub(r"(?<![一-鿿])(?:以弗所書|弗)\s*\d+(?::\d+(?:[-–]\d+)?)?", "", line)
