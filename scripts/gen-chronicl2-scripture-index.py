#!/usr/bin/env python3
"""Generate the 2 Chronicles volume's cross-book Scripture Index from the book's own text.

WHY THIS EXISTS
---------------
Same rationale as scripts/gen-numbers-scripture-index.py: a hand-curated
...
WHAT IT DOES
------------
Walks the source files in the order scripts/build-chronicl2-consolidated.sh
concatenates them, extracts every NON-2-Chronicles scripture reference, and
writes a Markdown table (reference -> chapters of this book) between the two
marker comments in 98-appendix-indices.md. Keyed to chapters, not page numbers.

Reuses the book-name tables and reference parser of the John generator.

Usage:  python3 scripts/gen-chronicl2-scripture-index.py --write   # update appendix
        python3 scripts/gen-chronicl2-scripture-index.py           # print table
        python3 scripts/gen-chronicl2-scripture-index.py --stats
"""
...
SOURCES = [
    ...
    ("00a-chronicles2-position.md",    "卷首·定位"),
    ("00b-seeking-spine.md",           "卷首·骨幹"),
    ("elder-wong-systematic-study.md", "卷首·領受總綱"),
] + [(f"{i:02d}", f"第{i}章") for i in range(1, 37)] + [
    ("99-open-door.md",                "卷末"),
    ("999-afterword.md",               "跋"),
]
    ...
    for fname, label in SOURCES:
        ...
        for line in p.read_text(encoding="utf-8").splitlines():
            if line.lstrip().startswith(SKIP_PREFIX):
                continue
            # 2 Chronicles itself is the book; drop its own explicit
            # self-references ("代下14:4", "歷代志下 1:7") before parsing
            # cross-book hits. 歷代志下/代下 are unambiguous multi-char
            # aliases, safe to strip wherever they appear.
            cleaned = re.sub(r"歷代志下\s*\d+(?::\d+(?:[-–]\d+)?)?", "", line)
            cleaned = re.sub(r"代下\s*\d+(?::\d+(?:[-–]\d+)?)?", "", cleaned)
