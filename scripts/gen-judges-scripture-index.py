#!/usr/bin/env python3
"""Generate the Judges volume's cross-book Scripture Index from the book's own text.
...
WHAT IT DOES
------------
Walks the source files in the order scripts/build-judges-consolidated.sh
concatenates them, extracts every NON-Judges scripture reference, and writes a
...
Usage:  python3 scripts/gen-judges-scripture-index.py --write   # update appendix
        python3 scripts/gen-judges-scripture-index.py           # print table
        python3 scripts/gen-judges-scripture-index.py --stats
"""
...
BOOKDIR = ROOT / "books" / "bible" / "judges"
...
# Judges cites itself as a bare chapter:verse (e.g. "8:1"), which the REF
# regex only picks up when prefixed by a book name/alias — so a bare "8:1" is
# already safe. What needs stripping is explicit self-references like
# "士 8:1" or "士師記 12:1", which would otherwise parse as a cross-book hit
# on 士師記 itself. Mid-sentence citations preceded by another CJK character
# (e.g. "如士師記8:1所記") are NOT stripped, matching gen-job-scripture-index.py's
# convention — those survive as legitimate intra-book cross-references, since
# they point a later chapter back to an earlier one worth navigating to.
...
SOURCES = [
    ...
    ("00a-judges-position.md",         "卷首·定位"),
    ("00b-cycle-spine.md",             "卷首·骨幹"),
    ("elder-wong-systematic-study.md", "卷首·領受總綱"),
] + [(f"{i:02d}", f"第{i}章") for i in range(1, 22)] + [
    ("99-no-king-but-you.md",          "卷末"),
    ("999-afterword.md",               "跋"),
]
    ...
    for fname, label in SOURCES:
        ...
        for line in p.read_text(encoding="utf-8").splitlines():
            if line.lstrip().startswith(SKIP_PREFIX):
                continue
            # Judges itself is the book; drop its own explicit self-references
            # (e.g. "士8:1", "士師記 12:1") before parsing cross-book hits.
            cleaned = re.sub(r"(?<![一-鿿])(?:士師記|士)\s*\d+(?::\d+(?:[-–]\d+)?)?", "", line)
