#!/usr/bin/env python3
"""Generate the Job volume's cross-book Scripture Index from the book's own text.
...
WHAT IT DOES
------------
Walks the source files in the order scripts/build-job-consolidated.sh
concatenates them, extracts every NON-Job scripture reference, and writes a
...
Usage:  python3 scripts/gen-job-scripture-index.py --write   # update appendix
        python3 scripts/gen-job-scripture-index.py           # print table
        python3 scripts/gen-job-scripture-index.py --stats
"""
...
# Job cites itself as a bare chapter:verse (e.g. "19:25-27"), which the REF
# regex only picks up when prefixed by a book name/alias — so a bare "19:25"
# is already safe. What needs stripping is explicit self-references like
# "伯 19:25" or "約伯記 3:1", which would otherwise parse as a cross-book hit
# on 約伯記 itself. "伯" is also a common transliteration syllable (伯大尼,
# 伯利恆, 伯示巴...) but those never have a bare digit immediately after, so
# no extra ambiguity guard is needed beyond the self-reference strip.
    ...
    for fname, label in SOURCES:
        ...
        for line in p.read_text(encoding="utf-8").splitlines():
            if line.lstrip().startswith(SKIP_PREFIX):
                continue
            # Job itself is the book; drop its own explicit self-references
            # (e.g. "伯19:25", "約伯記 3:1") before parsing cross-book hits.
            cleaned = re.sub(r"(?<![一-鿿])(?:約伯記|伯)\s*\d+(?::\d+(?:[-–]\d+)?)?", "", line)
