...
OUTPUT_DIR="${MARK_STAGE_DIR:?Use build-publication.sh; staging directory required}"
COMBINED_MD="$OUTPUT_DIR/gospel-of-mark-consolidated.md"
OUTPUT_PDF="$OUTPUT_DIR/gospel-of-mark-consolidated.pdf"
TEMPLATE="${MARK_TEMPLATE:?Staged template required}"
...
copyright: |
  ...
  **經文版權聲明 (Scripture Copyright Notices)**
  ...
  本版為編校校樣，尚未獲准發行（包括教會內部及非賣品贈閱）。版權審核及授權完成後，方可決定發行範圍；公開發行時另行申請 ISBN。
...
add_file() {
    local f="$1"
    [ -s "$f" ] || { echo "ERROR: Missing source: $f" >&2; exit 1; }
    ...
    # sed streams cloud-compressed source files reliably; tail can block on
    # dataless macOS files and leave a partial combined manuscript.
    sed '1,7d; s/\^\([0-9][0-9:]*\)\^/\\textsuperscript{\1}/g' "$f" >> "$COMBINED_MD"
    printf '\n\n\\newpage\n\n' >> "$COMBINED_MD"
...
add_chapter() {
    ...
    echo "ERROR: Missing chapter $1" >&2
    exit 1
}
...
add_volume "序言 · 預備道路——僕人登場 (Prologue) · 1:1-13" \
    "十三節，三幕戲：被預告、被印證、被試驗。僕人不需要家譜——第一章第九節，祂已經站在約但河裏了。**啟示的次序·第一至二步**：作者先向讀者交了底（1:1「神的兒子」），受洗時父也對子說了（1:11「你是我的愛子」）——兩處都在讀者與子之間，還沒有一個地上的人聽見。" \
    "**進院門**——曠野的人聲替主開路" \
    "1:1「神的兒子」——作者向讀者交底" \
...
add_volume "第一部 · 加利利事工——僕人的服事 (Galilean Ministry) · 1:14-8:26" \
    "服事的展開，衝突的升高，身分的追問——「這到底是誰？」八章的服事，就為逼出一個答案。**啟示的次序·第三步**：這一部裏唯一說對「神的兒子」的，是被喝令封口的污鬼（3:11-12）——說對的人，還說錯了時候。" \
    "**洗濯盆**——醫病、趕鬼、餵飽、趕路" \
    "εὐθύς「立刻」——僕人的步伐" \
    "來 7:25「他是長遠活著，替他們祈求」"
...
add_volume "第二部 · 走向耶路撒冷——僕人的道路 (The Way to Jerusalem) · 8:27-10:52" \
    "書脊在這裏：彼得答對了名字，答錯了意思。三次預言，三次跌倒，三堂僕人的課——結在 10:45。**啟示的次序·第四步**：山上的父再開一次口（9:7「這是我的愛子，你們要聽他」），對象換成了門徒——聽見的人，還是聽不懂十字架。" \
    "**從盆到壇的那段路**——兩個瞎子作前後的括號" \
...
add_volume "第三部 · 受難與復活——僕人的犧牲與高升 (Passion & Resurrection) · 11:1-16:20" \
    "三年寫十章，八天寫六章——急的是服事，慢的是捨命。幔子裂開，百夫長說出全書等了十五章的那句話。**啟示的次序·第五至六步**：子在公會親口承認（14:61-62「我是」），百夫長在十架下第一個說對（15:39）——十字架之前，誰說都會錯；十字架底下，連劊子手都能說對。" \
    "**祭壇**——祭牲上壇，幔子從上到下裂為兩半" \
...
sed '1,7d' "$INPUT_DIR/999-afterword.md" >> "$COMBINED_MD"
chapter_count=$((chapter_count + 1))
...
pandoc "$COMBINED_MD" \
  ...
  -V mark-cover-dir="$INPUT_DIR/publishing" \
  -V tocdepth=0 > "$LATEX_LOG" 2>&1
# Strict postflight and promotion are handled by publication.py.
